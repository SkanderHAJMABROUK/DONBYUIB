    ...
    if (this.aFormGroup.valid) {
      ...
      if (this.association) {
        const hashedPassword: string = sha256(
          this.aFormGroup.value.old_password + this.association.salt,
        ).toString();
        if (this.association.mdp == hashedPassword && this.association.id) {
          const hashedNewPassword: string = sha256(
            this.aFormGroup.value.new_password + this.association.salt,
          );
          this.updatePassword(
            this.association.id,
            hashedNewPassword,
        ...
        const hashedPassword: string = sha256(
          this.aFormGroup.value.old_password + this.donateur.salt,
        ).toString();
        if (this.donateur.mdp == hashedPassword && this.donateur.id) {
          const hashedNewPassword: string = sha256(
            this.aFormGroup.value.new_password + this.donateur.salt,
          );
          this.updatePassword(this.donateur.id, hashedNewPassword, 'Donateur')
            .then(() => {
              this.spinner.hide();
              this.aFormGroup.reset();
              this.showSuccessMessage = true;
