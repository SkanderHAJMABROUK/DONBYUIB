...
export class ModifierCollecteComponent {
  ...
  ngOnInit(): void {
    if (this.collecte.id) {
      this.service
        .checkPendingModificationDemand(this.collecte.id)
        .subscribe((isPending) => {
          this.isModificationDemandPending = isPending;
          this.initializeForm();
          if (this.collecte.id) {
            this.service
              .getModificationDateByCollecteId(this.collecte.id)
    ...
    this.collecteForm = this.formBuilder.group(
      {
        ...
        description: [
          {
            value: this.collecte?.description || '',
            disabled: this.isModificationDemandPending,
          },
          Validators.required,
        ],
        image: [
          {
            value: this.collecte?.image || '',
            disabled: this.isModificationDemandPending,
          },
          Validators.required,
        ],
        montant: [
          { value: this.collecte?.montant || '', disabled: true },
          Validators.required,
        ],
        date_debut: [
          {
  ...
  async onSubmit(): Promise<void> {
    if (
      ...
      JSON.stringify(this.collecteForm.getRawValue())
    ) {
