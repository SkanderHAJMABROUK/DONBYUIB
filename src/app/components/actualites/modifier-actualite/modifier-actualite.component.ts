...
export class ModifierActualiteComponent implements OnInit{
  ...
  ngOnInit(): void {
    if (this.actualite.id) {
      this.service.checkPendingModificationDemand(this.actualite.id).subscribe(isPending => {
          this.isModificationDemandPending = isPending;
          this.initializeForm();
          if (this.actualite.id) {
              this.service.getModificationDateByActualiteId(this.actualite.id).subscribe(modificationDate => {
  ...
  async onSubmit(): Promise<void> {
    if (JSON.stringify(this.initialValues) === JSON.stringify(this.actualiteForm.getRawValue())) {
