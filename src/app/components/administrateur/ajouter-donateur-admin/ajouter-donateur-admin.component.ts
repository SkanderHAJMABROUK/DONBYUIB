...
export class AjouterDonateurAdminComponent {
    ...
    this.aFormGroup = this.formBuilder.group(
      {
        ...
        date_de_naissance: [
          '',
          [Validators.required, this.serviceDonateur.dateOfBirthValidator()],
        ],
  ...
  async onSubmit(): Promise<void> {
    if (this.aFormGroup.valid) {
      const emailExists = await this.serviceDonateur
      ...
      if (!emailExists) {
        ...
        this.router.navigate(['/admin/ajouterDonateur/profil'], {
          replaceUrl: true,
        });
      } else {
