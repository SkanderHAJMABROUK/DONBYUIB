...
export class CompteAdminComponent implements OnInit {
  ...
  async getAssociationsByCategory() {
    try {
      ...
      this.associationsByCategory = Object.keys(categoryCounts).map(
        (category) => ({
          category: category,
        ...
        this.service.getAllDonCollecte().subscribe(
          (donCollecte) => {
            if (donAssociation.length === 0 && donCollecte.length === 0) {
              console.log(
                'No data available in DonAssociation and DonCollecte collections.',
              );
              return;
            }
            this.donationsData = this.aggregateDonationsData(
              donAssociation,
              donCollecte,
            );
  ...
  aggregateDonationsData(
    donAssociation: DonAssociation[],
    donCollecte: DonCollecte[],
  ): { date: Date; associationDonations: number; collecteDonations: number }[] {
  ...
  aggregateDonationsPerAssociation(
    ...
    return forkJoin(observables).pipe(
      map((associationIds: (string | undefined)[]) => {
        donAssociations.forEach((donAssociation: DonAssociation) => {
          ...
          aggregatedData[associationId] =
            (aggregatedData[associationId] || 0) + donAssociation.montant;
        });
        ...
        donCollectes.forEach((donCollecte: DonCollecte, index: number) => {
          ...
          if (associationId) {
            aggregatedData[associationId] =
              (aggregatedData[associationId] || 0) + donCollecte.montant;
          } else {
            console.log(
              'Error: Association ID not found for DonCollecte:',
              donCollecte,
            );
          }
        });
  ...
  renderDoughnutChart(): void {
    this.service.getAllDonAssociation().subscribe(
      (donAssociations: DonAssociation[]) => {
        this.service.getAllDonCollecte().subscribe(
          (donCollectes: DonCollecte[]) => {
            this.aggregateDonationsPerAssociation(
              donAssociations,
              donCollectes,
              ...
              const labelsPromises = associationIds.map((id) =>
                this.getAssociationNameById(id),
              );
          ...
          (donCollectes: DonCollecte[]) => {
            this.aggregateDonationsPerAssociation(
              ...
              const sortedDonations = Object.entries(aggregatedData)
                .map(([associationId, totalDonation]) => ({
                  associationId,
                  totalDonation,
                }))
              ...
              const labelsPromises = sortedDonations.map(({ associationId }) =>
                this.getAssociationNameById(associationId),
              );
              ...
              Promise.all(labelsPromises)
                .then((labels) => {
                  ...
                  this.allDonations = sortedDonations.map(
                    ({ totalDonation }, index) => ({
  ...
  getDashboardData(): void {
    this.analyse.getSatisfactionRate().subscribe(
      ...
      (error) => {
        console.log(
          'Erreur lors de la récupération du taux de satisfaction :',
          error,
        );
      },
    );
  ...
  aggregateDonationsPerDonator(
    ...
    donAssociations.forEach((donation: DonAssociation) => {
      ...
      if (donatorId) {
        aggregatedData[donatorId] =
          (aggregatedData[donatorId] || 0) + donation.montant;
      }
    });
