  ...
  compareFields(): void {
    if (this.demande.id_collecte) {
      this.collecteService
        .getCollecteById(this.demande.id_collecte)
        .subscribe((existingCollecte) => {
          if (existingCollecte) {
            const commonFields: (keyof Collecte &
              keyof DemandeModificationCollecte)[] = [
              'nom',
              'image',
              'description',
              'date_debut',
        ...
        if (this.demande.id_collecte) {
          ...
          if (modifiedField) {
                          ...
                          this.associationService
                            ...
                            .subscribe((associationName) => {
                              if (associationName) {
                                ...
                                const id_collecte = this.demande.id_collecte
                                  ? this.demande.id_collecte
                                  : '';
                                if (id_collecte) {
                                  this.collecteService
                                    .getCollecteNameById(id_collecte)
            ...
            this.collecteService
              .updateCollecteField(
                this.demande.id_collecte,
                label as keyof Partial<Collecte>,
