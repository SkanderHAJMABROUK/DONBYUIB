...
export class DemandesActualitesComponent implements OnInit {
  ...
  afficherDetails(actualite: DemandeActualite) {
    if (actualite.id) {
      this.actualiteService
        .getDemandeActualiteById(actualite.id)
        .subscribe((response) => {
  ...
  getActualites(): void {
    this.actualiteService
      .getPendingDemandesActualites()
      .subscribe((demandesActualites) => {
  ...
  chercherActualite(): void {
    ...
    this.filteredDemandeActualiteList = this.demandesActualites.filter(
      (demandeActualite, index) =>
        index >= startIndex &&
        index < endIndex &&
        demandeActualite.titre
          .toLowerCase()
    ...
    switch (this.selectedTri) {
      case 'plusRecents':
        this.filteredDemandeActualiteList =
          this.filteredDemandeActualiteList.sort(
            (a, b) =>
          ...
          this.updateDemandeEtat(selectedDemandeActualite.id, 'refusé')
            .then(() => {
              ...
              if (selectedDemandeActualite.id_actualite) {
                this.actualiteService
                  .deleteActualiteById(selectedDemandeActualite.id_actualite)
                  .then(() => {
                    if (selectedDemandeActualite.id) {
