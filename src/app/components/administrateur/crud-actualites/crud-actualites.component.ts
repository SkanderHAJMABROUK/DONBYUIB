...
export class CrudActualitesComponent implements OnInit {
  ...
  chercherActualite(): void {
    ...
    this.filteredActualiteList = this.filteredActualiteList.slice(
      startIndex,
      endIndex,
    );

    this.filteredActualiteList = this.actualites.filter(
      (actualite, index) =>
        index >= startIndex &&
        index < endIndex &&
        actualite.titre.toLowerCase().includes(this.searchTerm.toLowerCase()) &&
        (!this.selectedAssociation ||
          this.getAssociationNameById(actualite.id_association) ===
            this.selectedAssociation),
    );
    ...
    switch (this.selectedTri) {
      case 'plusRecents':
        this.filteredActualiteList = this.filteredActualiteList.sort(
          (a, b) =>
  ...
  supprimerActualite(actualite: Actualite) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.adminService
          .deleteActualiteByAdmin(actualite)
          .then(() => {
            Swal.fire({
              title: 'Supprimé !',
