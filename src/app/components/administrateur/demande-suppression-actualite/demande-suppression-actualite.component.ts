...
export class DemandeSuppressionActualiteComponent {
  ...
  searchTerm: string = '';
  selectedDemandeSuppressionActualite: DemandeSuppressionActualite =
    {} as DemandeSuppressionActualite;
  pageSize: number = 10;
  ...
  getDemandesSuppressionActualites(): void {
    this.actualiteService
      .getPendingDemandesSuppressionActualites()
      .subscribe((demandesSuppressionActualites) => {
  ...
  afficherDetails(actualite: DemandeSuppressionActualite) {
    if (actualite.id_actualite) {
      this.actualiteService
        .getActualiteById(actualite.id_actualite)
        .subscribe((response) => {
  ...
  chercherActualite(): void {
    ...
    this.filteredDemandeSuppressionActualiteList =
      this.demandesSuppressionActualites.filter(
        (demandeActualite, index) =>
          index >= startIndex &&
          index < endIndex &&
          (!this.searchTerm ||
            this.getActualiteNameById(demandeActualite.id_actualite)
              ?.toLowerCase()
              .includes(this.searchTerm.toLowerCase())) &&
          (!this.selectedAssociation ||
            this.getAssociationNameById(demandeActualite.id_association) ===
              this.selectedAssociation),
      );
    ...
    switch (this.selectedTri) {
      case 'plusRecents':
        this.filteredDemandeSuppressionActualiteList =
          this.filteredDemandeSuppressionActualiteList.sort(
  ...
  getActualitesIds(): void {
    this.actualitesIds = Array.from(
      new Set(
        this.demandesSuppressionActualites
          .map((demandeActualite) => demandeActualite.id_actualite)
          .filter(
            (id_actualite) =>
              id_actualite !== undefined && id_actualite !== null,
          ),
      ),
    ) as string[];
  ...
  refuserSuppressionActualite(
    selectedDemandeActualite: DemandeSuppressionActualite,
  ): void {
    Swal.fire({
      ...
      if (result.isConfirmed && selectedDemandeActualite.id) {
        ...
        if (selectedDemandeActualite.id) {
          this.updateDemandeEtat(selectedDemandeActualite.id, 'refusé')
            .then(() => {
              console.log(selectedDemandeActualite.id_actualite);
              if (selectedDemandeActualite.id_actualite) {
                this.updateActualiteEtat(
                  selectedDemandeActualite.id_actualite,
                  'accepté',
                )
                  .then(() => {
                    Swal.fire({
                    ...
                    if (
                      selectedDemandeActualite &&
                      selectedDemandeActualite.id_association
                    ) {
  ...
  accepterSuppressionActualite(
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (selectedDemandeActualite.id) {
          this.updateDemandeEtat(selectedDemandeActualite.id, 'accepté')
            .then(() => {
              ...
              if (selectedDemandeActualite.id_actualite) {
                this.updateActualiteEtat(
                  ...
                  .catch((error) => {
                    console.error(
                      "Erreur lors de la mise à jour de l'état de l'actualité:",
                      error,
                    );
                    Swal.fire({
                      title: 'Erreur',
