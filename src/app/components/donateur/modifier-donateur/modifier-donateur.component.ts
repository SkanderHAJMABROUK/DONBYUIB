...
export class ModifierDonateurComponent {
  ...
  onSubmit() {
    ...
    if (this.aFormGroup.valid) {
      ...
      this.service
        .modifierCompte({ id: this.donateurId, ...this.aFormGroup.value })
        .then(() => {
          console.log(
            'Données du donateur modifiées avec succès dans Firebase Firestore.',
          );
        ...
        .catch((error) => {
          console.error(
            'Erreur lors de la modification des données du donateur dans Firebase Firestore:',
            error,
          );
        });
    } else {
