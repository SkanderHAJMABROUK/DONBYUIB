...
export class EmailVerificationComponent implements OnInit {
  constructor(
    ...
    public aService: AssociationService,
    ...
    if (hashedEnteredCode === storedCode) {
      ...
      if (type === 'association') {
        const associationData = JSON.parse(
          localStorage.getItem('associationData') || '{}',
        );

        this.aService
          .addAssociationAndDemande(associationData)
          .then(() => {
            console.log(
              "Données de l'association ajoutées avec succès dans Firebase Firestore.",
            );
          ...
          .catch((error) => {
            console.error(
              "Erreur lors de l'ajout des données de l'association dans Firebase Firestore:",
              error,
            );
            let email: string | null = localStorage.getItem('emailAssociation');
        ...
        this.dService
          .ajouterDonateur(userData)
          .then(() => {
            console.log(
              'Données du donateur ajoutées avec succès dans Firebase Firestore.',
            );
            // Réinitialiser le formulaire après l'ajout des données
          ...
          .catch((error) => {
            console.error(
              "Erreur lors de l'ajout des données du donateur dans Firebase Firestore:",
              error,
            );
            let email: string | null = localStorage.getItem('emailDonateur');
      ...
      if (this.remainingAttempts === 0) {
        Swal.fire({
          icon: 'error',
          title:
            'La vérification de votre adresse email est échoué! Veuillez saisir une adresse email valide.',
          showConfirmButton: false,
          timer: 3000,
        });
