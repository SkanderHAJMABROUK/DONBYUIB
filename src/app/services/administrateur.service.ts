...
export class AdministrateurService {
  ...
  modifierAssociation(assocation: Association): Promise<void> {
    ...
    let associationRef = this.firestore
    ...
    return this.getAdminByLoginAndPassword(login, password).pipe(
      map((admin) => {
        if (admin) {
          ...
          this.route.navigate(['/admin/profil', admin.id]); // Rediriger avec l'ID de l'administrateur dans l'URL
  ...
  getAllDonAssociation(): Observable<DonAssociation[]> {
    return this.firestore
      .collection<DonAssociation>('DonAssociation')
      .valueChanges();
  }
  ...
  getAllDonCollecteCollecteIds(): Observable<{ [key: string]: string }> {
    return this.firestore
      .collection<DonCollecte>('DonCollecte')
      .valueChanges()
      .pipe(
        map((dons_collectes) => {
  ...
  async sendAcceptationNotification(
  ...
  async sendModificationResultNotification(
    email: string | undefined,
