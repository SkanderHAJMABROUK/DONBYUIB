...
export class CollecteService {
    ...
    return collectionData(collecteCollection, { idField: 'id' }).pipe(
      map((collectes: any[]) => {
        return collectes.map((collecte) => ({
          ...
          date_debut:
            collecte.date_debut instanceof Timestamp
              ? collecte.date_debut.toDate()
              : collecte.date_debut,
          date_fin:
            collecte.date_fin instanceof Timestamp
              ? collecte.date_fin.toDate()
              : collecte.date_fin,
      ...
      map((demandesCollectes: any[]) => {
        return demandesCollectes
          ...
          .map((demandeCollecte) => ({
            ...
            date_debut:
              demandeCollecte.date_debut instanceof Timestamp
                ? demandeCollecte.date_debut.toDate()
                : demandeCollecte.date_debut,
            date_fin:
              demandeCollecte.date_fin instanceof Timestamp
                ? demandeCollecte.date_fin.toDate()
                : demandeCollecte.date_fin,
            date:
              demandeCollecte.date instanceof Timestamp
                ? demandeCollecte.date.toDate()
                : demandeCollecte.date,
          }));
      }),
    );
  }
  ...
  getDemandesModificationsCollectes(): Observable<
    DemandeModificationCollecte[]
  > {
    let demandeModificationCollectesCollection = collection(
      this.fs,
      'DemandeModificationCollecte',
    );
    return collectionData(demandeModificationCollectesCollection, {
      idField: 'id',
    }).pipe(
      map((demandeModificationCollecte: any[]) => {
        return demandeModificationCollecte.map(
          (demandeModificationCollecte) => ({
            ...
            date:
              demandeModificationCollecte.date instanceof Timestamp
                ? demandeModificationCollecte.date.toDate()
                : demandeModificationCollecte.date,
          }),
        );
      }),
    );
  ...
  getPendingDemandesModificationsCollectes(): Observable<
    DemandeModificationCollecte[]
  > {
    let demandeModificationCollectesCollection = collection(
      this.fs,
    ...
    return collectionData(demandeModificationCollectesCollection, {
      ...
      map((demandeModificationCollecte: any[]) => {
        return demandeModificationCollecte
          .filter(
            (demandeModificationCollecte) =>
              demandeModificationCollecte.etat === 'en_attente',
          )
  ...
  getAcceptedDemandesModificationsCollectes(): Observable<
    DemandeModificationCollecte[]
  > {
    let demandeModificationCollectesCollection = collection(
      this.fs,
    ...
    return collectionData(demandeModificationCollectesCollection, {
      ...
      map((demandeModificationCollecte: any[]) => {
        return demandeModificationCollecte
          .filter(
            (demandeModificationCollecte) =>
              demandeModificationCollecte.etat === 'accepté',
          )
          .map((demandeModificationCollecte) => ({
            id: demandeModificationCollecte.id,
            id_association: demandeModificationCollecte.id_association,
  ...
  getDemandesSuppressionCollectes(): Observable<DemandeSuppressionCollecte[]> {
    let demandeSuppressionCollectesCollection = collection(
      this.fs,
      'DemandeSuppressionCollecte',
    );
    return collectionData(demandeSuppressionCollectesCollection, {
      idField: 'id',
    }).pipe(
      map((demandeSuppressionCollecte: any[]) => {
        return demandeSuppressionCollecte.map((demandeSuppressionCollecte) => ({
          ...
          date:
            demandeSuppressionCollecte.date instanceof Timestamp
              ? demandeSuppressionCollecte.date.toDate()
              : demandeSuppressionCollecte.date,
        }));
      }),
    );
  }

  getPendingDemandesSuppressionCollectes(): Observable<
    DemandeSuppressionCollecte[]
  > {
    let demandeSuppressionCollectesCollection = collection(
      this.fs,
    ...
    return collectionData(demandeSuppressionCollectesCollection, {
      ...
      map((demandeSuppressionCollecte: any[]) => {
        return demandeSuppressionCollecte
          .filter(
            (demandeSuppressionCollecte) =>
              demandeSuppressionCollecte.etat === 'en_attente',
          )
  ...
  getAcceptedDemandesSuppressionCollectes(): Observable<
    DemandeSuppressionCollecte[]
  > {
    let demandeSuppressionCollectesCollection = collection(
      this.fs,
    ...
    return collectionData(demandeSuppressionCollectesCollection, {
      ...
      map((demandeSuppressionCollecte: any[]) => {
        return demandeSuppressionCollecte
          .filter(
            (demandeSuppressionCollecte) =>
              demandeSuppressionCollecte.etat === 'accepté',
          )
          .map((demandeSuppressionCollecte) => ({
            id: demandeSuppressionCollecte.id,
            id_association: demandeSuppressionCollecte.id_association,
  ...
  getDemandeModificationCollecteById(
    id: string,
  ): Observable<DemandeModificationCollecte | undefined> {
  ...
  async ajouterCollecteAndDemande(collecteData: Collecte) {
    ...
    const collecteDocRef = await addDoc(
      collection(this.fs, 'Collecte'),
      collecteToAdd,
    );
  ...
  getModificationDateByCollecteId(
    collecteId: string,
  ...
  updateCollecteField(
    id: string,
    fieldName: keyof Partial<Collecte>,
    newValue: any,
  ): Promise<void> {
    const collecteRef = this.firestore.collection('Collecte').doc(id);
  ...
  getCollecteNameById(id: string): Observable<string | undefined> {
    return this.getCollecteById(id).pipe(map((collecte) => collecte?.nom));
  }

  updateCumulativeDonationAmount(
    collecteId: string,
