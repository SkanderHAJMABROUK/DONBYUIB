...
export class ActualiteService {
    ...
    return collectionData(actualiteCollection, { idField: 'id' }).pipe(
      map((actualites: any[]) => {
        return actualites.map((actualite) => ({
          ...
          date_publication:
            actualite.date_publication instanceof Timestamp
              ? actualite.date_publication.toDate()
      ...
      map((demandesActualites: any[]) => {
        return demandesActualites
          ...
          .map((demandeActualite) => ({
            ...
            date_publication:
              demandeActualite.date_publication instanceof Timestamp
                ? demandeActualite.date_publication.toDate()
                : demandeActualite.date_publication,
  ...
  getDemandesModificationsActualites(): Observable<
    DemandeModificationActualite[]
  > {
    let demandeModificationActualitesCollection = collection(
      this.fs,
      'DemandeModificationActualite',
    );
    return collectionData(demandeModificationActualitesCollection, {
      idField: 'id',
    }).pipe(
      map((demandeModificationActualite: any[]) => {
        return demandeModificationActualite.map(
          (demandeModificationActualite) => ({
            ...
            date:
              demandeModificationActualite.date instanceof Timestamp
                ? demandeModificationActualite.date.toDate()
                : demandeModificationActualite.date,
          }),
        );
      }),
    );
  ...
  getPendingDemandesModificationsActualites(): Observable<
    DemandeModificationActualite[]
  > {
    let demandeModificationActualitesCollection = collection(
      this.fs,
    ...
    return collectionData(demandeModificationActualitesCollection, {
      ...
      map((demandeModificationActualite: any[]) => {
        return demandeModificationActualite
          .filter(
            (demandeModificationActualite) =>
              demandeModificationActualite.etat === 'en_attente',
          )
  ...
  getAcceptedDemandesModificationsActualites(): Observable<
    DemandeModificationActualite[]
  > {
    let demandeModificationActualitesCollection = collection(
      this.fs,
    ...
    return collectionData(demandeModificationActualitesCollection, {
      ...
      map((demandeModificationActualite: any[]) => {
        return demandeModificationActualite
          .filter(
            (demandeModificationActualite) =>
              demandeModificationActualite.etat === 'accepté',
          )
          .map((demandeModificationActualite) => ({
            id: demandeModificationActualite.id,
            id_association: demandeModificationActualite.id_association,
  ...
  getDemandesSuppressionActualites(): Observable<
    DemandeSuppressionActualite[]
  > {
    let demandeSuppressionActualitesCollection = collection(
      this.fs,
      'DemandeSuppressionActualite',
    );
    return collectionData(demandeSuppressionActualitesCollection, {
      idField: 'id',
    }).pipe(
      map((demandeSuppressionActualite: any[]) => {
        return demandeSuppressionActualite.map(
          (demandeSuppressionActualite) => ({
            ...
            date:
              demandeSuppressionActualite.date instanceof Timestamp
                ? demandeSuppressionActualite.date.toDate()
                : demandeSuppressionActualite.date,
          }),
        );
      }),
    );
  ...
  getPendingDemandesSuppressionActualites(): Observable<
    DemandeSuppressionActualite[]
  > {
    let demandeSuppressionActualitesCollection = collection(
      this.fs,
    ...
    return collectionData(demandeSuppressionActualitesCollection, {
      ...
      map((demandeSuppressionActualite: any[]) => {
        return demandeSuppressionActualite
          .filter(
            (demandeSuppressionActualite) =>
              demandeSuppressionActualite.etat === 'en_attente',
          )
  ...
  getAcceptedDemandesSuppressionActualites(): Observable<
    DemandeSuppressionActualite[]
  > {
    let demandeSuppressionActualitesCollection = collection(
      this.fs,
    ...
    return collectionData(demandeSuppressionActualitesCollection, {
      ...
      map((demandeSuppressionActualite: any[]) => {
        return demandeSuppressionActualite
          .filter(
            (demandeSuppressionActualite) =>
              demandeSuppressionActualite.etat === 'accepté',
          )
          .map((demandeSuppressionActualite) => ({
            id: demandeSuppressionActualite.id,
            id_association: demandeSuppressionActualite.id_association,
  ...
  getDemandeModificationActualiteById(
    id: string,
  ): Observable<DemandeModificationActualite | undefined> {
    ...
    return this.getActualites().pipe(
      map((actualites) =>
        actualites.filter(
          ...
          (actualite.etat === 'accepté' || actualite.etat === 'en_attente_de_suppression'),
        ),
      ),
    );
  }
  ...
  async ajouterActualiteAndDemande(actualiteData: Actualite) {
    ...
    const actualiteDocRef = await addDoc(
      collection(this.fs, 'Actualite'),
      actualiteToAdd,
    );
  ...
  getDemandeActualiteById(
    id: string,
  ): Observable<DemandeActualite | undefined> {
  ...
  supprimerActualite(actualite: Actualite): Observable<void> {
    ...
    return new Observable<void>((observer) => {
      this.firestore
        .collection('DemandeSuppressionActualite')
        .add(demandeSuppression)
        .then(() => {
          observer.next();
          observer.complete();
        })
  ...
  checkPendingModificationDemand(actualiteId: string): Observable<boolean> {
    return this.firestore
      .collection<DemandeSuppressionActualite>(
        ...
        (ref) =>
          ref
            .where('id_actualite', '==', actualiteId)
            .where('etat', '==', 'en_attente'),
      )
      .valueChanges()
      .pipe(map((demands) => demands.length > 0));
  }
  ...
  getModificationDateByActualiteId(
    actualiteId: string,
        ...
        map((demands) => {
          if (demands.length > 0) {
            const modificationDate =
              demands[0].date instanceof Timestamp
                ? demands[0].date.toDate()
                : demands[0].date;
            const day = modificationDate.getDate().toString().padStart(2, '0');
            const month = (modificationDate.getMonth() + 1)
              .toString()
              .padStart(2, '0'); // Notez que getMonth() retourne les mois de 0 à 11
  ...
  modifierActualiteByAdmin(actualite: Actualite): Promise<void> {
    ...
    const collecteRef = this.firestore
  ...
  updateActualiteField(
    id: string,
    fieldName: keyof Partial<Actualite>,
    newValue: any,
  ): Promise<void> {
    const actualiteRef = this.firestore.collection('Actualite').doc(id);
  ...
  getActualiteTitleById(id: string): Observable<string | undefined> {
    return this.getActualiteById(id).pipe(map((collecte) => collecte?.titre));
  }
}
