...
export class AssociationService {
  ...
  nomAssociation: string | undefined =
    localStorage.getItem('nomAssociation') || '';
    ...
    return collectionData(demandeAssociationCollection, { idField: 'id' }).pipe(
      map((demandesAssociations: any[]) => {
        return demandesAssociations.map((demandeAssociation) => ({
          ...
          date:
            demandeAssociation.date instanceof Timestamp
  ...
  getDemandesModificationsAssociations(): Observable<
  ...
  > {
    ...
    return collectionData(demandeModificationAssociationCollection, {
      ...
      map((demandeModificationAssociation: any[]) => {
        return demandeModificationAssociation.map(
          (demandeModificationAssociation) => ({
  ...
  getPendingDemandesModificationsAssociations(): Observable<
    DemandeModificationAssociation[]
  > {
    let demandeModificationAssociationCollection = collection(
      this.fs,
  ...
  getAcceptedDemandesModificationsAssociations(): Observable<
    DemandeModificationAssociation[]
  > {
    let demandeModificationAssociationCollection = collection(
      this.fs,
    ...
    return this.getAssociationByEmailAndPassword(email, password).pipe(
      map((association) => {
        if (association) {
          ...
          localStorage.setItem('nomAssociation', association.nom);
          this.route.navigate(['/login/profilAssociation', association.id], {
            replaceUrl: true,
          });

          localStorage.setItem('associationId', association.id || '');
  ...
  checkEmailExists(email: string): Observable<boolean> {
    return this.firestore
      .collection('Association', (ref) => ref.where('email', '==', email))
      .get()
  ...
  checkNameExists(nom: string): Observable<boolean> {
    return this.firestore
      .collection('Association', (ref) => ref.where('nom', '==', nom))
      .get()
      .pipe(map((snapshot) => !snapshot.empty));
  }
  ...
  getDemandDateByIdAssociation(
    ...
    return this.firestore
      ...
      .pipe(
        map((demands) => {
          if (demands.length > 0) {
            return demands[0].date instanceof Timestamp
              ? demands[0].date.toDate()
              : demands[0].date;
          } else {
            return undefined;
